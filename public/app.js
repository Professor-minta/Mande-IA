...
const preferences = { theme: localStorage.getItem('mande-theme') || 'light', language: localStorage.getItem('mande-language') || 'bm', voice: localStorage.getItem('mande-voice') === 'true' };
...
async function say(content, button) {
  ...
  try {
    ...
    player.onended=()=>{ if (state.activeAudio===player) { resetSpeechButton(state.activeSpeechButton); state.activeAudio=null; state.activeSpeechButton=null; $('#status').textContent='Écrivez en Bambara — micro bêta'; } };
    await player.play();
