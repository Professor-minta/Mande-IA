...
const preferences = { theme: localStorage.getItem('mande-theme') || 'dark', language: ['auto', 'bm', 'fr', 'en'].includes(savedLanguage) ? savedLanguage : 'auto', voice: localStorage.getItem('mande-voice') === 'true' };
...
function applyPreferences() { document.documentElement.dataset.theme = preferences.theme; document.querySelector('meta[name="theme-color"]')?.setAttribute('content', preferences.theme === 'dark' ? '#0b0f16' : '#113c37'); $('#theme-toggle').checked = preferences.theme === 'dark'; $('#language-select').value = preferences.language; $('#voice-toggle').checked = preferences.voice; $('#voice-description').textContent = state.djelia ? 'Moussa lira la réponse dans sa langue' : 'Disponible après configuration de Djelía'; }
